...
def getTransformationFromCamToArm(api):
    ...
    def getdCalibrationBoardCorners():
    ...
    def getCornersWithArm(xOffset, yOffset):
        ...
        cornersToBoard3D[:,0] = -np.matmul(np.expand_dims(lengthSpan, -1), np.ones((1, horizontalAxis))).reshape(-1) + xOffset
        cornersToBoard3D[:,1] = -np.matmul(np.ones((verticalAxis, 1)), np.expand_dims(widthSpan, 0)).reshape(-1) + yOffset
